$('#sub_cat').on('pageshow',function(event){
	var s=sessionStorage.getItem("cat_id");
		//alert(s);
		$("#list").empty();
		$("#secheader").empty();
		...
		xmlhttp.open("GET","http://mycity91.com/APIS/files/getsubcategory_new.php?catid="+s+"&pageno="+pageno+"&cityid="+city,false);
		...
		var name=xmlDoc.getElementsByTagName("categoryname")[i].firstChild.data;
		var namel=name.replace(/#/g,"&");
		document.getElementById("list").innerHTML+= "<li ><a href='javascript:link2("+xmlDoc.getElementsByTagName("id")[i].firstChild.data+")'>"+namel+"</a></li>";
		}
